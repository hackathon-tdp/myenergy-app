...
const AddNewLocalisation = ({}) => {
	...
	const addlocalisationHandler = async () => {
		const { data } = await API.get(`organizations/?name=${user.email}`);
		API.post("locations/", { name, price, organization: data[0].id });
	};
	...
	return (
		<View style={{ paddingVertical: 60, alignItems: "center" }}>
			...
			<Input
				marginTop={18}
				placeholder="Cena za 1kwh w złotówkach"
...
export const AddForm = ({ navigation, visible, toggle }) => {
	const AddFromStack = createStackNavigator();
	...
	const Stages = [
		...
		[
			null,
			<AddNewLocalisation stage={stage} />,
			<FormCompleted type="lokalizację" />,
			null,
		],
	...
	// 						>
	// 							{option != 0 && Stages[option][stage + 1] == null
	// 								? "Zakończ"
	// 								: "Dalej"}
	// 						</Button>
	// 					</InlineBox>
			...
			<View style={styles.modal}>
				<Wrapper backgroundColor="transparent">
					<AddFromStack.Navigator>
						...
						/>
						<AddFromStack.Screen
							name="addLocalisationForm"
							component={AddNewLocalisation}
							options={{ headerShown: false }}
						/>
						<AddFromStack.Screen
							name="addLocalisationCompleted"
							component={FormCompleted}
							options={{ headerShown: false }}
							screenOptions={{ type: "lokalizację" }}
						/>
						<AddFromStack.Screen
							name="addDeviceForm"
						...
						<AddFromStack.Screen
							name="choseHoursForm"
							component={ChooseHours}
							options={{ headerShown: false }}
						/>
						<AddFromStack.Screen
							name="addDeviceCompleted"
							component={FormCompleted}
							options={{ headerShown: false }}
							screenOptions={{ type: "urządzenie" }}
						/>
					</AddFromStack.Navigator>
				</Wrapper>
			</View>
